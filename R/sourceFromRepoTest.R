...
## usage:
## repo <- getRepo("/user/MyProject")
## sfr(repo, "my/special/script.R")
## # objects in global environment
## e <- new.env()
## sfr(repo, "my/special/script.R", local=e)
## # objects in environment e
sfr <- function(repository, repositoryPath, ...) {
  ...
  # note: is there a way to get RCurl to return a "file-like" connection object
